...
nodes_data = {}
with open('apps_utils/deployment/nodes.json') as reader:
...
def on_request(request: Dict):
    ...
    if method == 'get_random_seed':
        ...
        if not MuonUtils.verify_deployment_signature(seed, message):
            raise Exception('Deployment: Seed is not verified')
        calculated_party = MuonUtils.get_new_random_subset(all_nodes, int(seed, 16), n)
        if not set(party).issubset(set(calculated_party)):
            raise Exception('Deployment: Calculated party is not subset of the given party.')
        ...
        raise Exception('Deployment: The number of verified peerIds is less than threshold.')
